...
function hardDelNote(id) {
  ...
  ST.delete('postit', id);
  if (typeof syncDeleteFromFirebase === 'function') syncDeleteFromFirebase('postit', id);
  toast('🗑️ ลบถาวรแล้ว');
  render();
}
...
function emptyNoteTrash() {
  ...
  trash.forEach(function(n) {
    ST.delete('postit', n.id);
    if (typeof syncDeleteFromFirebase === 'function') syncDeleteFromFirebase('postit', n.id);
  });
