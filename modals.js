...
function showPipelineM(dealerId, eid) {
  ...
  openM(eid ? '✏️ Pipeline' : '➕ เพิ่ม Pipeline', '' +
    ...
    '<div class="fg"><label>DJI Authorized Dealer *</label><input type="text" id="fp_dealer" list="fp_dealer_list" value="' + sanitize((ST.getOne('dealers', dealerId || p.dealerId) || {}).name || '') + '" placeholder="บริษัทในเมนู Dealer — พิมพ์อิสระ หรือเลือกจาก suggest" autocomplete="off">' + _dealerNameDatalistHtml('fp_dealer_list') + '</div></div>' +
    '<div class="fr"><div class="fg"><label>Sub Dealer</label><input type="text" id="fp_djid" list="fp_djid_list" value="' + sanitize(p.djiDealer || '') + '" placeholder="บริษัทย่อยที่ยื่นประมูลใต้ Authorized Dealer (ไม่ผูกกับเมนู Dealer)" autocomplete="off">' + _djiDealerDatalistHtml('fp_djid_list') + '</div>' +
...
// คัดลอกขั้นตอนทั้งหมดเป็น bullet list (พร้อมหมายเหตุต่อข้อถ้ามี) — เอาไปวางสรุปใน Visit Report ได้เลย
