...
function showPipelineM(dealerId, eid) {
  ...
  openM(eid ? '✏️ Pipeline' : '➕ เพิ่ม Pipeline', '' +
    ...
    '<div class="fg"><label>DJI Dealer *</label><input type="text" id="fp_dealer" list="fp_dealer_list" value="' + sanitize((ST.getOne('dealers', dealerId || p.dealerId) || {}).name || '') + '" placeholder="บริษัทที่เข้าประมูล พิมพ์อิสระ หรือเลือกจาก suggest" autocomplete="off">' + _dealerNameDatalistHtml('fp_dealer_list') + '</div></div>' +
    '<div class="fr"><div class="fg"><label>Dealer Name</label><input type="text" id="fp_djid" list="fp_djid_list" value="' + sanitize(p.djiDealer || '') + '" placeholder="พิมพ์อิสระ หรือเลือกจาก suggest" autocomplete="off">' + _djiDealerDatalistHtml('fp_djid_list') + '</div>' +
