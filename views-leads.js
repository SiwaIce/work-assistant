...
var LEAD_FIELD_TYPES = [
  ...
  {v:'imageLink',  l:'🔗 รูปภาพคลิกได้ (เปิดลิงก์)'},
  {v:'date',       l:'วันที่'},
...
function _openLeadModal(f) {
  ...
  var formMode = (f && f.formMode) || 'survey';
  h += '<div class="fg"><label>ประเภทฟอร์ม</label><div style="display:flex;gap:8px">';
  h += '<label style="flex:1;display:flex;align-items:flex-start;gap:6px;padding:10px;border:1px solid var(--border,#334155);border-radius:8px;cursor:pointer;background:var(--bg,#0f172a)"><input type="radio" name="lf_mode" value="survey" ' + (formMode === 'survey' ? 'checked' : '') + ' style="margin-top:2px"><span><b style="font-size:.85em;display:block">📋 แบบสอบถาม</b><span style="font-size:.72em;color:var(--text2)">มีปุ่มส่งข้อมูล เก็บเป็น Lead</span></span></label>';
  h += '<label style="flex:1;display:flex;align-items:flex-start;gap:6px;padding:10px;border:1px solid var(--border,#334155);border-radius:8px;cursor:pointer;background:var(--bg,#0f172a)"><input type="radio" name="lf_mode" value="catalog" ' + (formMode === 'catalog' ? 'checked' : '') + ' style="margin-top:2px"><span><b style="font-size:.85em;display:block">🖼️ หน้าเลือกรายการ</b><span style="font-size:.72em;color:var(--text2)">ไม่มีปุ่มส่ง ไม่เก็บข้อมูล — ใช้สแกน QR ไปโบรชัวร์/ลิงก์</span></span></label>';
  h += '</div></div>';
  ...
  var descStyle = (f && f.descStyle) || {};
  h += '<div class="fg"><label>📝 คำอธิบายฟอร์ม</label><textarea id="lf_desc" class="fm-input" rows="3" placeholder="รายละเอียดหรือคำชี้แจงสำหรับลูกค้า">' + esc(description) + '</textarea>' + _lfStyleControlsHtml('lf_desc_style', descStyle) + '</div>';
  ...
  h += '<div class="fg"><label>📐 การจัดข้อความ <small style="color:var(--text2)">(หัวฟอร์ม / คำอธิบาย / หัวข้อ Section)</small></label><select id="lf_text_align" class="fm-input">';
  h += '<option value="default"' + (textAlign === 'default' ? ' selected' : '') + '>ค่าเริ่มต้น</option>';
  h += '<option value="left"' + (textAlign === 'left' ? ' selected' : '') + '>⬅️ ชิดซ้ายทั้งหมด</option>';
  h += '<option value="center"' + (textAlign === 'center' ? ' selected' : '') + '>↔️ กึ่งกลางทั้งหมด</option>';
  h += '</select></div>';
  h += '</div>'; // pane-design
...
function _lfSecHtml(sec) {
  ...
  fields.forEach(function(f, idx) {
    ...
    h += '</div>';
    if (isSec) h += _lfStyleControlsHtml('lf_fstyle_' + f.id, f.style, '_lfSetStyle(\'' + sec + '\',\'' + f.id + '\',\'lf_fstyle_' + f.id + '\')');
    if (isImg) {
...
// ตัวควบคุมสไตล์ข้อความ (ตัวหนา/ตัวเอียง/ขนาด) ใช้ร่วมกันทั้งคำอธิบายฟอร์ม (global) และหัวข้อ Section (ต่อ field)
// idPrefix ต้องไม่ซ้ำกัน ใช้อ่านค่าตอน save (คำอธิบายฟอร์ม) หรือ onchange ทันที (per-field ผ่าน _lfSetStyle)
function _lfStyleControlsHtml(idPrefix, style, onChangeCall) {
  style = style || {};
  var oc = onChangeCall ? ' onchange="' + onChangeCall + '"' : '';
  var h = '<div style="display:flex;gap:10px;align-items:center;margin-top:6px;font-size:.78rem;color:var(--text2)">';
  h += '<label style="display:flex;align-items:center;gap:3px;cursor:pointer"><input type="checkbox" id="' + idPrefix + '_b"' + (style.bold ? ' checked' : '') + oc + '> <b>ตัวหนา</b></label>';
  h += '<label style="display:flex;align-items:center;gap:3px;cursor:pointer"><input type="checkbox" id="' + idPrefix + '_i"' + (style.italic ? ' checked' : '') + oc + '> <i>ตัวเอียง</i></label>';
  h += '<label style="display:flex;align-items:center;gap:3px">ขนาด <select id="' + idPrefix + '_s" style="font-size:.78rem;padding:2px 4px"' + oc + '>';
  [['normal', 'ปกติ'], ['large', 'ใหญ่'], ['xlarge', 'ใหญ่มาก'], ['small', 'เล็ก']].forEach(function(o) {
    h += '<option value="' + o[0] + '"' + ((style.size || 'normal') === o[0] ? ' selected' : '') + '>' + o[1] + '</option>';
  });
  h += '</select></label></div>';
  return h;
}
function _lfReadStyleInputs(idPrefix) {
  var b = document.getElementById(idPrefix + '_b');
  var i = document.getElementById(idPrefix + '_i');
  var s = document.getElementById(idPrefix + '_s');
  return { bold: b ? b.checked : false, italic: i ? i.checked : false, size: s ? s.value : 'normal' };
}
function _lfSetStyle(sec, id, idPrefix) {
  var f = _lfGet(sec, id); if (!f) return;
  f.style = _lfReadStyleInputs(idPrefix);
}
...
// re-render field list โดยคงสถานะเปิด/ปิดของ panel ที่กำลังแก้ไขอยู่ไว้ — กันปัญหา panel หุบเวลาเพิ่ม/ลบ/ย้ายรายการ
function _lfRerenderFields(s) {
  var openIds = Array.prototype.slice.call(document.querySelectorAll('.lf-fbody.open')).map(function(el) { return el.id.replace('lfbody_', ''); });
  document.getElementById('lf_fields_wrap').innerHTML = _lfSecHtml(s);
  openIds.forEach(function(id) {
    var body = document.getElementById('lfbody_' + id);
    var chev = document.getElementById('lf-chev-' + id);
    if (body) body.classList.add('open');
...
// สลับลำดับรายการภายใน field เดียวกัน (ไม่ใช่สลับลำดับ field — ดู _lfMoveF สำหรับอันนั้น)
function _lfOptSwap(s, id, i, dir) {
  var f = _lfGet(s, id); if (!f || !f.options) return;
  var to = i + dir;
  if (to < 0 || to >= f.options.length) return;
  var tmp = f.options[i]; f.options[i] = f.options[to]; f.options[to] = tmp;
  _lfRerenderFields(s);
}
function _lfOptMoveBtns(sec, fid, i, len) {
  return '<div style="display:flex;flex-direction:column;gap:2px;flex-shrink:0">' +
    '<button type="button" class="btn bsm bo" style="padding:1px 6px;font-size:9px" onclick="_lfOptSwap(\'' + sec + '\',\'' + fid + '\',' + i + ',-1)"' + (i === 0 ? ' disabled' : '') + '>▲</button>' +
    '<button type="button" class="btn bsm bo" style="padding:1px 6px;font-size:9px" onclick="_lfOptSwap(\'' + sec + '\',\'' + fid + '\',' + i + ',1)"' + (i === len - 1 ? ' disabled' : '') + '>▼</button>' +
    '</div>';
}
// คัดลอกรายการเดี่ยว (แทรกต่อท้ายตัวที่ก็อป) — ใช้ร่วมกันทั้ง multicheck และ imageLink
function _lfOptDup(s, id, i) {
  var f = _lfGet(s, id); if (!f || !f.options) return;
  var copy = JSON.parse(JSON.stringify(f.options[i]));
  if (copy && typeof copy === 'object') copy.label = (copy.label || '') + ' (สำเนา)';
  f.options.splice(i + 1, 0, copy);
  _lfRerenderFields(s);
}
function _lfMcOptsHtml(sec, f) {
  var opts = f.options || [];
  var h = '<div class="lf-opts-wrap"><small style="color:var(--text2);">ตัวเลือก + รูปภาพ (ไม่บังคับ)</small>';
  h += '<button type="button" class="btn bsm bo" style="margin-top:6px;width:100%" onclick="showLfBulkImportM(\'' + sec + '\',\'' + f.id + '\')">📋 วางหลายรายการทีเดียว</button>';
  opts.forEach(function(o, i) {
    ...
    h += _lfOptMoveBtns(sec, f.id, i, opts.length);
...
function _lfImgLinkOptsHtml(sec, f) {
  var opts = f.options || [];
  var h = '<div class="lf-opts-wrap"><small style="color:var(--text2);">รายการรูปภาพ + ลิงก์ปลายทาง (คลิกที่รูปแล้วเปิดลิงก์ทันที ไม่นับเป็นคำถาม)</small>';
  h += '<button type="button" class="btn bsm bo" style="margin-top:6px;width:100%" onclick="showLfPresetLoadM(\'' + sec + '\',\'' + f.id + '\')">📂 โหลดชุดที่บันทึกไว้</button>';
  opts.forEach(function(o, i) {
    var lbl = (o && o.label) || '';
    var img = (o && o.img) || '';
    var link = (o && o.link) || '';
    h += '<div style="display:flex;flex-wrap:wrap;gap:5px;margin-top:6px;align-items:center;padding:6px;border:1px solid var(--border,#334155);border-radius:6px">';
    h += _lfOptMoveBtns(sec, f.id, i, opts.length);
  ...
  h += '<button type="button" class="btn bsm bo" style="flex:1" onclick="_lfIlAdd(\'' + sec + '\',\'' + f.id + '\')">+ เพิ่มรายการ</button>';
  h += '<button type="button" class="btn bsm bo" style="flex:1" onclick="showLfBulkImportM(\'' + sec + '\',\'' + f.id + '\')">📋 วางหลายรายการทีเดียว</button>';
  h += '<button type="button" class="btn bsm bo" style="flex:1" onclick="_lfSaveImgLinkPreset(\'' + sec + '\',\'' + f.id + '\')">💾 บันทึกชุดนี้</button>';
  h += '</div>';
  h += '</div>';
  return h;
}
// วางหลายรายการทีเดียว (paste จาก Excel/Sheets แบบ tab-separated หรือพิมพ์เอง คั่นด้วย comma) — ใช้ได้ทั้ง multicheck และ imageLink
function showLfBulkImportM(sec, fieldId) {
  var f = _lfGet(sec, fieldId);
  var isIl = f && f.type === 'imageLink';
  var h = '<div>';
  h += '<div style="font-size:.8rem;color:var(--text2);margin-bottom:8px">วางข้อมูลจาก Excel/Sheets ได้เลย (คอลัมน์: ชื่อ' + (isIl ? ', URL รูปภาพ, ลิงก์ปลายทาง' : ', URL รูปภาพ') + ') หรือพิมพ์เอง 1 บรรทัด = 1 รายการ</div>';
  h += '<textarea id="lfBulkTa" class="fm-input" rows="8" placeholder="' + (isIl ? 'Matrice 4 Series\thttps://cdn.../m4.jpg\thttps://dji.com/m4' : 'ชื่อสินค้า\thttps://cdn.../img.jpg') + '"></textarea>';
  h += '<div style="display:flex;gap:8px;margin-top:10px">';
  h += '<button class="btn bp" style="flex:1" onclick="_lfBulkApply(\'' + sec + '\',\'' + fieldId + '\')">➕ เพิ่มรายการเหล่านี้</button>';
  h += '<button class="btn bo" onclick="closeMForce()">ยกเลิก</button>';
  h += '</div></div>';
  openM('📋 วางหลายรายการทีเดียว', h);
}
function _lfBulkApply(s, id) {
  var f = _lfGet(s, id); if (!f) return;
  var raw = (document.getElementById('lfBulkTa').value || '').trim();
  if (!raw) { closeMForce(); return; }
  var isIl = f.type === 'imageLink';
  var added = 0;
  raw.split('\n').forEach(function(line) {
    ...
    if (!label) return;
    var img = (parts[1] || '').trim();
    if (isIl) f.options.push({ label: label, img: img, link: (parts[2] || '').trim() });
    else f.options.push({ label: label, img: img });
    added++;
  });
  closeMForce();
  _lfRerenderFields(s);
  toast('➕ เพิ่ม ' + added + ' รายการแล้ว');
}
// ---------- Preset: ชุดรูปภาพ+ลิงก์ ที่บันทึกไว้ใช้ซ้ำข้ามฟอร์ม (เก็บใน cfg เหมือนค่าตั้งค่าอื่นๆ — sync ผ่าน saveConfig ตามปกติ) ----------
function _lfSaveImgLinkPreset(s, id) {
  var f = _lfGet(s, id); if (!f) return;
  if (!f.options || !f.options.length) { toast('⚠️ ยังไม่มีรายการให้บันทึก'); return; }
  var name = prompt('ตั้งชื่อชุดรายการนี้ เช่น "สินค้า DJI 2026"');
  if (!name || !name.trim()) return;
  var cfg = getConfig();
  cfg.lfImgLinkPresets = cfg.lfImgLinkPresets || [];
  cfg.lfImgLinkPresets.push({ id: _lfId(), name: name.trim(), options: JSON.parse(JSON.stringify(f.options)), savedAt: new Date().toISOString() });
  saveConfig(cfg);
  toast('💾 บันทึกชุด "' + name.trim() + '" แล้ว');
}
// popover แยกต่างหากจาก modal หลัก (ไม่ใช้ openM/closeMForce) — เพราะปุ่มนี้กดจากภายใน modal
// "แก้ไข Lead Form" ที่เปิดอยู่แล้ว ถ้าใช้ openM() ซ้อนจะไปเขียนทับเนื้อหา modal เดิม (มี #mBd ตัวเดียวทั้งแอป)
// และถ้า apply preset แล้ว closeMForce() จะปิด modal แก้ไข Lead Form ทั้งหมดไปด้วย ไม่ใช่แค่ popover นี้
function showLfPresetLoadM(s, id) {
  var cfg = getConfig();
  var presets = cfg.lfImgLinkPresets || [];
  var h;
  if (!presets.length) {
    h = '<div style="text-align:center;padding:20px;color:var(--text2)">ยังไม่มีชุดที่บันทึกไว้ — บันทึกได้จากปุ่ม "💾 บันทึกชุดนี้" ใต้รายการ</div>';
  } else {
    h = '<div style="max-height:340px;overflow-y:auto">';
    presets.forEach(function(p) {
      h += '<div style="display:flex;align-items:center;gap:8px;padding:8px;border:1px solid var(--border,#334155);border-radius:8px;margin-bottom:6px">';
      h += '<div style="flex:1;min-width:0"><b style="font-size:.86em">' + esc(p.name) + '</b><div style="font-size:.72em;color:var(--text2)">' + (p.options || []).length + ' รายการ</div></div>';
      h += '<button type="button" class="btn bsm bo" onclick="_lfApplyPreset(\'' + s + '\',\'' + id + '\',\'' + p.id + '\',false)">แทนที่</button>';
      h += '<button type="button" class="btn bsm bo" onclick="_lfApplyPreset(\'' + s + '\',\'' + id + '\',\'' + p.id + '\',true)">เพิ่มต่อ</button>';
      h += '<button type="button" class="btn bsm bd" onclick="_lfDeletePreset(\'' + p.id + '\',\'' + s + '\',\'' + id + '\')">🗑️</button>';
      h += '</div>';
    });
    h += '</div>';
  }
  _lfPresetPopoverShow('📂 เลือกชุดรายการที่บันทึกไว้', h);
}
function _lfPresetPopoverShow(title, bodyHtml) {
  _lfPresetPopoverClose();
  var ov = document.createElement('div');
  ov.id = 'lfPresetOv';
  ov.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:400;display:flex;align-items:center;justify-content:center;padding:16px';
  ov.onclick = function(e) { if (e.target === ov) _lfPresetPopoverClose(); };
  var box = document.createElement('div');
  box.style.cssText = 'background:var(--card,#1e293b);border:1px solid var(--border,#334155);border-radius:12px;max-width:480px;width:100%;max-height:80vh;overflow-y:auto';
  box.innerHTML =
    '<div style="display:flex;justify-content:space-between;align-items:center;padding:12px 14px;border-bottom:1px solid var(--border,#334155)">' +
    '<b style="font-size:.9em">' + title + '</b><button type="button" onclick="_lfPresetPopoverClose()" style="background:none;border:none;color:var(--text2);font-size:1rem;cursor:pointer">✕</button></div>' +
    '<div style="padding:14px">' + bodyHtml + '</div>';
  ov.appendChild(box);
  document.body.appendChild(ov);
}
function _lfPresetPopoverClose() {
  var ov = document.getElementById('lfPresetOv');
  if (ov) ov.remove();
}
function _lfApplyPreset(s, id, presetId, append) {
  var f = _lfGet(s, id); if (!f) return;
  var cfg = getConfig();
  var preset = (cfg.lfImgLinkPresets || []).find(function(p) { return p.id === presetId; });
  if (!preset) return;
  var cloned = JSON.parse(JSON.stringify(preset.options || []));
  f.options = append ? (f.options || []).concat(cloned) : cloned;
  _lfPresetPopoverClose();
  _lfRerenderFields(s);
  toast((append ? '➕ เพิ่มต่อชุด "' : '📂 แทนที่ด้วยชุด "') + preset.name + '" แล้ว');
}
function _lfDeletePreset(presetId, s, id) {
  if (!confirm('ลบชุดนี้?')) return;
  var cfg = getConfig();
  cfg.lfImgLinkPresets = (cfg.lfImgLinkPresets || []).filter(function(p) { return p.id !== presetId; });
  saveConfig(cfg);
  showLfPresetLoadM(s, id);
}
function _lfIlSetLbl(s, id, i, v)  { var f = _lfGet(s, id); if (!f) return; var o = f.options[i] || {}; f.options[i] = { label: v, img: o.img || '', link: o.link || '' }; }
function _lfIlSetImg(s, id, i, v)  { var f = _lfGet(s, id); if (!f) return; var o = f.options[i] || {}; f.options[i] = { label: o.label || '', img: v, link: o.link || '' }; }
function _lfIlSetLink(s, id, i, v) { var f = _lfGet(s, id); if (!f) return; var o = f.options[i] || {}; f.options[i] = { label: o.label || '', img: o.img || '', link: v }; }
function _lfIlAdd(s, id) {
  var f = _lfGet(s, id); if (!f) return;
  f.options.push({ label: '', img: '', link: '' });
  _lfRerenderFields(s);
}
function _lfIlDel(s, id, i) {
  var f = _lfGet(s, id); if (!f) return;
  f.options.splice(i, 1);
  _lfRerenderFields(s);
}
...
function saveLeadForm() {
  ...
  var data = {
    ...
    textAlign:      (document.getElementById('lf_text_align') ? document.getElementById('lf_text_align').value : 'default'),
    descStyle:      _lfReadStyleInputs('lf_desc_style'),
  };
...
function showLeadFormDetail(formId) {
  ...
  db.collection(LEAD_FORM_COL).doc(formId).get().then(function(fd) {
    ...
    Promise.all([
      db.collection(LEAD_SUB_COL).where('formId', '==', formId).orderBy('createdAt', 'desc').get(),
      db.collection('leadFormClicks').where('formId', '==', formId).get().catch(function() { return {forEach: function() {}}; })
    ]).then(function(results) {
      var subs = [];
      results[0].forEach(function(d) { subs.push(Object.assign({id: d.id}, d.data())); });
      var clicks = [];
      results[1].forEach(function(d) { clicks.push(Object.assign({id: d.id}, d.data())); });
      _ldCache = {form: form, subs: subs, clicks: clicks};
      window._ldAllSubs = subs;
      window._ldForm    = form;
      el.innerHTML = _ldAnalyticsHtml(form, subs, clicks);
    });
  });
}

// สรุปจำนวนคลิกต่อรายการ (จาก field ชนิด imageLink) — ไม่ใช่ submission แยกนับต่างหาก
// จัดกลุ่มนับคลิกต่อรายการ — ใช้ร่วมกันทั้งตอนแสดงผลใน analytics และตอน export Excel
function _ldClickSummary(clicks) {
  var byItem = {};
  (clicks || []).forEach(function(c) {
    var key = (c.fieldLabel || '') + '||' + (c.itemLabel || '');
    if (!byItem[key]) byItem[key] = {fieldLabel: c.fieldLabel || '', itemLabel: c.itemLabel || '', itemLink: c.itemLink || '', count: 0, last: null};
    byItem[key].count++;
    var d = c.createdAt && c.createdAt.toDate ? c.createdAt.toDate() : (c.createdAt ? new Date(c.createdAt) : null);
    if (d && (!byItem[key].last || d > byItem[key].last)) byItem[key].last = d;
  });
  return Object.values(byItem).sort(function(a, b) { return b.count - a.count; });
}
function _ldImgLinkClicksHtml(clicks) {
  if (!clicks || !clicks.length) return '';
  var rows = _ldClickSummary(clicks);
  var h = '<div class="ld-chart-card" style="margin-bottom:16px">';
  h += '<div class="ld-chart-title">🔗 คลิกดูสินค้า (รูปภาพ+ลิงก์) — รวม ' + clicks.length + ' ครั้ง</div>';
  h += '<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:.82rem">';
  h += '<thead><tr style="text-align:left;color:var(--text2,#94a3b8)"><th style="padding:5px 8px">รายการ</th><th style="padding:5px 8px">Field</th><th style="padding:5px 8px;text-align:right">จำนวนคลิก</th><th style="padding:5px 8px">คลิกล่าสุด</th></tr></thead><tbody>';
  rows.forEach(function(r) {
    h += '<tr style="border-top:1px solid var(--border,#334155)">';
    h += '<td style="padding:5px 8px">' + esc(r.itemLabel || '-') + (r.itemLink ? ' <a href="' + esc(r.itemLink) + '" target="_blank" style="font-size:.85em;opacity:.7">🔗</a>' : '') + '</td>';
    h += '<td style="padding:5px 8px;color:var(--text2,#94a3b8)">' + esc(r.fieldLabel || '-') + '</td>';
    h += '<td style="padding:5px 8px;text-align:right;font-weight:700">' + r.count + '</td>';
    h += '<td style="padding:5px 8px;color:var(--text2,#94a3b8)">' + (r.last ? _ldFmtDate(r.last) : '-') + '</td>';
    h += '</tr>';
  });
...
function _ldAnalyticsHtml(form, subs, clicks) {
  ...
  if (subs.length || (clicks && clicks.length)) h += '<button onclick="exportLeadSubs()" class="btn bsm bp">📎 Export</button>';
  if (subs.length && AI_FEATURES_ENABLED) h += '<button id="ldAiBtn" onclick="aiAnalyzeLeads(this)" class="btn bsm bo">🤖 AI</button>';
  h += '</div></div>';
...
function exportLeadSubs() {
  ...
  var clicks = _ldCache.clicks || [];
  if (clicks.length) {
    var clickRows = [['รายการ', 'Field', 'จำนวนคลิก', 'คลิกล่าสุด', 'ลิงก์ปลายทาง']];
    _ldClickSummary(clicks).forEach(function(r) {
      clickRows.push([r.itemLabel, r.fieldLabel, r.count, r.last ? _ldFmtDate(r.last) : '', r.itemLink]);
    });
    var wsClicks = XLSX.utils.aoa_to_sheet(clickRows);
