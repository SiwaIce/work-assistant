// ===== GOOGLE SHEET SYNC CONFIG (แอป → Sheet ทางเดียว, auto ทุกครั้งที่บันทึก Pipeline) =====
// โหลดค่าจริงจาก Firestore appConfig/sheetSync ตอน login (ดู firebase-sync.js) — ตั้งค่าที่
// ⚙️ ตั้งค่า → ☁️ เชื่อมต่อ → Google Sheet Sync ใช้งานจริงใน sheet-sync.js
var SHEET_SYNC_URL = '';
var SHEET_SYNC_SECRET = '';
var SHEET_SYNC_ENABLED = false;
...
// "archived" = จบแล้วจริงๆ ไม่ต้องติดตามต่อ (Fail&Lost, ส่งมอบแล้ว/Deliver) หรือถูก Hide ไว้เอง — ใช้ตัดสินว่า
// โครงการนี้ควรอยู่แท็บ "Archived Project" ตอน export/import (แยกจากแท็บ Main) และซ่อนจากรายการหลักในแอปโดย
// default — ไม่ใช่ field ใหม่ที่ต้องเก็บเพิ่ม คำนวณสดจาก status/sheetDisplay ที่มีอยู่แล้วเสมอ
// Win/Contracting (ยังไม่ถึง Deliver) ตั้งใจ "ไม่" archive อัตโนมัติ — ยัง Win แต่ยังไม่ส่งมอบ ทีมยังต้อง
// ติดตามต่อจนกว่าจะ Deliver จริง หรือกด Hide เองถ้าอยากซ่อนก่อนกำหนด
function pipeIsArchived(p) { return pipeIsLost(p) || p.status === 'deliver' || p.sheetDisplay === 'Hide'; }
...
// ================================================================
// HELP — FAQ สั้นๆ รวมฟีเจอร์ที่ผู้ใช้ใหม่มักหาไม่เจอ (ก่อนหน้านี้แอพไม่มี help ในตัวเลย มีแต่ title=
// tooltip บนปุ่มแต่ละจุด) ใช้ <details>/<summary> ของ HTML เอง ไม่ต้องเขียน JS accordion เพิ่ม
// ================================================================
function showHelpM() {
  var items = [
    ['🔍 ค้นหาแบบเร็ว (Ctrl+K)', 'กด Ctrl+K (หรือแตะ 🔍 มุมขวาบน) เพื่อค้นหาข้ามทุกเมนู — Dealer, Pipeline, ใบเสนอราคา, Sales Order, Stock, Lead'],
    ['☑️ เลือกหลายรายการ (Bulk actions)', 'หน้า Dealers / Pipeline / ใบเสนอราคา / Sales Order มีปุ่ม "☑️ เลือกหลายรายการ" — เลือกได้หลายแถวพร้อมกัน แล้วเปลี่ยนสถานะ/Export/ลบทีเดียว'],
    ['↩️ เลิกทำ (Undo) หลังลบ', 'ลบ Dealer / Pipeline / ใบเสนอราคา / Sales Order แล้ว จะมีแถบ "เลิกทำ" ค้างไว้ 5 วินาทีมุมล่างจอ กดเพื่อกู้คืนได้ทันที'],
    ['🟢 จุดสถานะ Sync (มุมขวาบน)', '🟢 = เชื่อมต่อ Cloud ปกติ, ⚪ = ยังไม่ได้เชื่อมต่อ, 🟡 = sync มีปัญหาล่าสุด, 🔴 = ออฟไลน์ — ข้อมูลจะบันทึกในเครื่องก่อนเสมอ ไม่หาย'],
    ['🎯 สิ่งที่ต้องทำวันนี้ (Smart Filter)', 'ป้ายสีบนหน้า Dealers/Pipeline (เช่น "🔥 5 Pipeline ค้างนาน") กดแล้วพาไปดูรายการที่ต้องรีบจัดการ ตรงกับที่หน้า "วันนี้" ใช้อยู่แล้ว'],
    ['🔗 เชื่อม Dealer ↔ ใบเสนอราคา/SO', 'หน้ารายละเอียด Dealer มีแท็บ "💰 ใบเสนอราคา" และ "📦 Sales Order" ดูของ Dealer นั้นได้ตรงๆ ไม่ต้องไปกรองเอง'],
    ['🔍 เช็ค Log ซ้ำ', 'หน้ารายละเอียด Pipeline มีปุ่ม "🔍 เช็ค Log ซ้ำ" เฉพาะโครงการนั้น หรือใช้ "🩺 ตรวจสุขภาพข้อมูล" หน้า Pipeline รวมสำหรับเช็คทุกโครงการทีเดียว'],
    ['🗂️ สลับมุมมอง Card/Table', 'หน้า Dealers / Pipeline / ใบเสนอราคา / Sales Order สลับดูแบบการ์ด (เหมาะมือถือ) หรือตาราง (เหมาะดูรวดเดียวเยอะๆ) ได้จากปุ่มมุมขวาบนของหน้า']
  ];
  var h = items.map(function(it) {
    return '<details style="margin-bottom:8px;border:1px solid var(--border);border-radius:10px;padding:10px 12px">' +
      '<summary style="cursor:pointer;font-weight:700;font-size:13.5px">' + it[0] + '</summary>' +
      '<div style="margin-top:8px;font-size:13px;color:var(--text2);line-height:1.6">' + it[1] + '</div></details>';
  }).join('');
  openM('❓ วิธีใช้งาน', h);
}
...
var APP_MENU_REGISTRY = [
  ...
  {id: 'teamVisitPlan', icon: '🧭', name: 'Visit Plan ทีม'},
...
var MB_NAV_ROUTE_GROUPS = {
  ...
  pipeline: ['pipeline', 'pipelineTeam', 'pipeBoard', 'pipeDash', 'pipeHealth', 'pipeDetail', 'mondayMeeting', 'mondayCompany', 'posCalibration', 'kpiCompanyPlan', 'pipelineCompare', 'forecast', 'forecastComparison', 'djiProjects', 'djiLedger', 'worklist'],
...
var ALL_QUICK_ITEMS = [
  ...
  {id:'worklist',    icon:'✅', name:'งานค้าง', action:"go('worklist')"},
